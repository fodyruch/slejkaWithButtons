...
if not TELEGRAM_TOKEN or not API_KEY:
    raise ValueError("Необходимо установить TELEGRAM_TOKEN и BATTLEMETRICS_API_KEY в файле .env")

# Глобальные переменные
user_sessions = {}  # {chat_id: {'tracked_players': {player_id: server_id}, 'status_messages': {player_id: message_id}, 'last_status': {player_id: bool}, 'main_message_id': int}}
    ...
    try:
        ...
        for chat_id, session in user_sessions.items():
            safe_data[str(chat_id)] = {
                ...
                'main_message_id': session.get('main_message_id', None)
            }
...
MENU_BUTTONS = [
    ...
    [InlineKeyboardButton("📋 Отслеживаемые игроки", callback_data="view_players")]
]


async def show_menu(chat_id: int, context: ContextTypes.DEFAULT_TYPE, message_id: int = None):
    keyboard = InlineKeyboardMarkup(MENU_BUTTONS)
    text = "🎮 *Меню отслеживания игроков*"
    ...
    if message_id:
        ...
        try:
            ...
            user_sessions[chat_id]['main_message_id'] = message.message_id
    else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if chat_id not in user_sessions:
        user_sessions[chat_id] = {'tracked_players': {}, 'status_messages': {}, 'last_status': {}, 'main_message_id': None}
    
    # Получаем message_id из update
    message_id = None
    ...
    await show_menu(chat_id, context, message_id)


# Функция для отображения списка отслеживаемых игроков
async def show_tracked_players(chat_id: int, context: ContextTypes.DEFAULT_TYPE, message_id: int):
    if chat_id not in user_sessions or not user_sessions[chat_id]['tracked_players']:
        text = "ℹ️ У вас нет отслеживаемых игроков"
    ...
    # Получаем актуальную информацию о всех игроках
    async with aiohttp.ClientSession() as session:
        api = BattleMetricsAPI(API_KEY, session)
        buttons = []
        text = "📋 Ваши отслеживаемые игроки:\n\n"

        for player_id, server_id in user_sessions[chat_id]['tracked_players'].items():
            # Получаем полную информацию
            player_info = await get_player_status_message(player_id, server_id, api)
            text += player_info + "\n\n"
            # Добавляем кнопку удаления
            delete_button = InlineKeyboardButton("🗑 Удалить", callback_data=f"remove_{player_id}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in user_sessions:
        user_sessions[chat_id] = {'tracked_players': {}, 'status_messages': {}, 'last_status': {}, 'main_message_id': message_id}
    else:
        user_sessions[chat_id]['main_message_id'] = message_id

    if query.data == "add_player":
        text = "Введите ID игрока (число):"
        # Установим состояние пользователя: ожидание ID
        user_sessions[chat_id]['waiting_for_player_id'] = True
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
        ...
        await show_tracked_players(chat_id, context, message_id)

    elif query.data.startswith("remove_"):
        player_id = query.data.split("_", 1)[1]
        
        if chat_id in user_sessions and player_id in user_sessions[chat_id]['tracked_players']:
            # Получаем имя игрока перед удалением для уведомления
            async with aiohttp.ClientSession() as session:
                api = BattleMetricsAPI(API_KEY, session)
                player_info = await api.get_player_info(player_id)
                player_name = player_info.get("data", {}).get("attributes", {}).get("name", f"ID:{player_id}")
            ...
            try:
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"✅ Игрок {player_name} (ID: {player_id}) успешно удален из отслеживания!"
                )
            except Exception as e:
...
# Обработчик текстовых сообщений (для ввода ID игрока)
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, ждем ли мы ID игрока
    if user_sessions[chat_id].get('waiting_for_player_id', False):
        player_id = update.message.text.strip()
        
        # Сбрасываем флаг ожидания
        user_sessions[chat_id]['waiting_for_player_id'] = False
        
        if not player_id.isdigit():
            text = "❌ ID игрока должно быть числом"
            try:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
                ...
                user_sessions[chat_id]['main_message_id'] = message.message_id
                await asyncio.sleep(2)
                await show_menu(chat_id, context, message.message_id)
            return

        if player_id in user_sessions[chat_id]['tracked_players']:
            text = "⚠️ Этот игрок уже отслеживается."
            try:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
        ...
        if len(user_sessions[chat_id]['tracked_players']) >= MAX_PLAYERS_PER_USER:
            text = f"🚫 Вы не можете отслеживать больше {MAX_PLAYERS_PER_USER} игроков."
            try:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if 'errors' in sessions:
                text = "❌ Игрок не найден или нет данных о сессиях"
                try:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message_id,
            ...
            if not server_id:
                text = "❌ Не удалось определить сервер для игрока"
                try:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message_id,
            ...
            message_text = await get_player_status_message(player_id, server_id, api)
            status_message = await update.message.reply_text(message_text)
            user_sessions[chat_id]['status_messages'][player_id] = status_message.message_id
        ...
        # Если пользователь отправил обычное сообщение, показываем меню
        await show_menu(chat_id, context, message_id)
